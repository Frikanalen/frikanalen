...
class FillJukeboxUnitTests(TestCase):
    ...
    def test_times_are_rounded(self):
        ...
        self.assertEqual([3, 1, 1, 3, 3], [r["id"] for r in res])
        self.assertEqual(self.start_date + datetime.timedelta(minutes=1), res[0]["starttime"])
    ...
    def test_cases(self):
        ...
        for name, video_str, sched_str, expect_str in tc:
            ...
            with self.subTest(name=name, videos=video_str, scheds=sched_str, dur=sched_cur):
                self.assertEqual(expect_str, newly_scheduled_str)
