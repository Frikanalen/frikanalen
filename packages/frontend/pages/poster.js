...
export default class PosterManager extends Component {
	...
	resetImageTimeout() {
		...
		const queryString = 'preview/?text=' + this.state.text + '&heading=' + this.state.heading
		...
		fetch('https://stills-generator.frikanalen.no/program',
			{
				...
				body: JSON.stringify({'text': this.state.text, 'heading': this.state.heading}),
				credentials: "include",
			}
		)
	}
      ...
      <WindowWidget>
	      <Modal.Dialog className="text-dark">
			...
			<Button variant="secondary">Avbryt</Button>
			      <Button variant="primary" onClick={() => this.uploadPoster()} >Last opp</Button>
		      </Modal.Footer>
	      </Modal.Dialog>
      </WindowWidget>
    </Layout>
