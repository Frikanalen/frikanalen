...
export default function Signupform(): JSX.Element {
  ...
  async function signup(e: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      if (fe instanceof Error) setErrorMessage(<p>Feil fra server: {fe.message}</p>);
    }
    try {
      const token = await getUserToken(email, password);
      if (userContext?.login) userContext.login(token);
    } catch (e) {
      if (e instanceof Error) setErrorMessage(<p>Feil fra server: {e.message}</p>);
    }
  }
          ...
          <Card.Body>
            ...
            <Card.Text>
              <Alert variant="info">
                ...
                <p>
                  For mer om medlemskap, se: «
                  <Link passHref href="/om/blimed">
                    <a>Bli med!</a>
                  </Link>
                  »
                </p>
